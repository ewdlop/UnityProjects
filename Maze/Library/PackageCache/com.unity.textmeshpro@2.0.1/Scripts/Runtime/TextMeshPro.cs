 ...
 public:
  ...
  dynamic_string& get_as_string(dynamic_string& dst) const {
    switch (m_type) {
      ...
      case cDTUInt:
        ...
        break;
      case cDTFloat:
        dst.format("%f", m_float);
        break;
      case cDTVec3F:
        dst.format("%f,%f,%f", (*m_pVec3F)[0], (*m_pVec3F)[1], (*m_pVec3F)[2]);
        break;
      case cDTVec3I:
  ...
  int serialize(void* pBuf, uint buf_size, bool little_endian) const {
    ...
    switch (m_type) {
      ...
      case cDTUInt:
      case cDTFloat: {
        if (!utils::write_obj(m_float, pBuf, buf_left, little_endian))
          return -1;
        break;
      }
      case cDTVec3F: {
  ...
  int deserialize(const void* pBuf, uint buf_size, bool little_endian) {
    ...
    switch (m_type) {
      ...
      case cDTUInt:
      case cDTFloat: {
        if (!utils::read_obj(m_float, pBuf, buf_left, little_endian))
          return -1;
        break;
      }
      case cDTVec3F: {
